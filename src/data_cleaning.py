...
def update_boroughs_in_dataframe(combined_df, zip_borough_map):
    """
    Updates boroughs in the combined dataframe using the zip-to-borough mapping.

    Parameters:
    combined_df (pd.DataFrame): The combined dataframe
    zip_borough_map (dict): A mapping of zip codes to boroughs

    Returns:
    pd.DataFrame: The combined dataframe with updated boroughs
    """
    return update_boroughs(combined_df, zip_borough_map)
...
def fill_missing_data(df, k):
    ...
    combined_df = update_boroughs_in_dataframe(combined_df, zip_borough_map)
